  ...
  'zh-CN': {
    roomTopology: {
      ...
      issues: {
        ...
        riskTitle: '{count} 个资源提醒',
        riskDescription: '按房间所在机器的实际运行情况评估，包含同机其他房间。',
        capacityOvercommitted: '机器“{machine}”当前运行 {count} 个世界，超过建议数量 {limit}；建议观察 CPU 占用和游戏流畅度。',
        capacityFull: '机器“{machine}”当前运行 {count} 个世界，已达到建议数量 {limit}；启动更多世界前请留意资源余量。',
        capacityUnknown: '机器“{machine}”缺少最新的运行数据，暂时无法评估当前容量。',
        memoryCritical: '机器“{machine}”当前可用内存不足 384 MiB，世界可能因内存不足退出。',
        memoryTight: '机器“{machine}”当前可用内存低于 768 MiB，建议错开更新、地图生成和压缩任务。',
  ...
  'en-US': {
    roomTopology: {
      ...
      issues: {
        ...
        riskTitle: '{count} resource notices',
        riskDescription: 'Based on current usage of this room’s machines, including other rooms on the same machines.',
        capacityOvercommitted: '“{machine}” currently runs {count} worlds, above the recommended {limit}; monitor CPU usage and game performance.',
        capacityFull: '“{machine}” currently runs {count} worlds, reaching the recommended {limit}; check resource headroom before starting more.',
        capacityUnknown: '“{machine}” has no current runtime data, so its current capacity cannot be assessed.',
        memoryCritical: '“{machine}” currently has less than 384 MiB of available memory; worlds may exit due to insufficient memory.',
        memoryTight: '“{machine}” currently has less than 768 MiB of available memory; stagger updates, map rendering, and compression tasks.',
