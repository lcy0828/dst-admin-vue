  ...
  <Card>
    ...
    <CardContent class="flex flex-col gap-5">
      ...
      <template v-if="!loading && snapshot">
        ...
        <FieldGroup>
          <Field v-for="field in fields" :key="field.id" :data-disabled="!field.editable || saving">
        ...
        <div v-if="checks.length" class="flex flex-col gap-3" aria-live="polite">
          <div v-for="check in checks" :key="check.id" class="flex flex-col gap-1">
            <div class="flex flex-wrap items-center justify-between gap-2">
              ...
              <Badge :variant="check.status === 'fail' ? 'destructive' : 'outline'">{{ t(`setup.checkStatus.${check.status}`) }}</Badge>
            </div>
            <p v-if="check.status !== 'pass' && check.remediation" class="break-words text-sm text-muted-foreground">{{ check.remediation }}</p>
          </div>
