<template>
  <Card :class="cn('topology-world', { 'is-master': master, 'is-error': hasError, 'is-warning': hasWarning })" size="sm">
    <CardHeader class="flex flex-row items-start gap-2.5">
      <span class="topology-world-icon" aria-hidden="true"><Trees v-if="worldType === 'forest'" /><Mountain v-else-if="worldType === 'cave'" /><Boxes v-else /></span>
...
import { Boxes, Mountain, Trees } from '@lucide/vue'
