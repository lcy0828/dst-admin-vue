...
async function getContext({ roomId = '', worldId = '' } = {}) {
  ...
  if (rooms.length === 0) throw new Error('没有已接管的房间，无法管理模组')

  let room = null
  if (roomId) {
    room = rooms.find(item => matchResource(item, roomId))
    if (!room) throw new Error(`未找到已接管的房间：${roomId}`)
  ...
  if (room) {
    ...
    if (worldId) {
      ...
      if (!world) throw new Error(`房间 ${room.name} 中不存在世界：${worldId}`)
...
async function searchMods({ keyword, page = 1, pageSize = 20 }) {
  const response = await modsV2API.search(requireValue(keyword?.trim(), '请输入搜索关键词'), page, pageSize)
  return {
    ...response,
...
async function downloadMod(input) {
  const roomId = requireValue(input.roomId, '请先选择房间')
  const modId = requireValue(input.modid || input.id, '缺少模组 ID')
...
async function getModConfig({ roomId, worldId, modid, mod = {} }) {
  requireValue(roomId, '请先选择房间')
  requireValue(worldId, '请先选择要配置的世界')
  const configuration = await modsV2API.configuration(roomId, worldId, requireValue(modid, '缺少模组 ID'))
  return {
    status: 200,
    modinfo: {
      ...mod,
...
async function saveModCustomConfig(input) {
  const roomId = requireValue(input.roomId, '请先选择房间')
  const worldId = requireValue(input.worldId, '请先选择要配置的世界')
  const modId = requireValue(input.modid, '缺少模组 ID')
  const request = {
    expectedRevision: requireValue(input.expectedRevision, '缺少当前配置版本，请重新打开配置'),
...
async function toggleMod(input) {
  const job = await modsV2API.enable(
    requireValue(input.roomId, '请先选择房间'),
    requireValue(input.modid, '缺少模组 ID'),
...
async function deleteMod(input) {
  const job = await modsV2API.uninstall(
    ...
    requireValue(input.modid, '缺少模组 ID'),
    {
      ...
      confirmation: requireValue(input.confirmation, '请输入完整房间名确认卸载'),
...
async function getAllModConfigFile({ roomId, worldId }) {
  const file = await modsV2API.configurationFile(
    requireValue(roomId, '请先选择房间'),
    requireValue(worldId, '请先选择要查看的世界')
  )
