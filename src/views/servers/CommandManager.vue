  ...
  <div>
    <el-card class="box-card command-execute-card">
      ...
      <el-form v-if="commandMode === 'structured'" :model="executeForm" label-width="120px">
        ...
        <template v-if="currentCommand && currentCommand.parameterized">
        ...
        <el-form-item v-if="!currentCommand || !currentCommand.parameterized">
...
<script>
import { commandManager, COMMAND_TYPES } from '@/api';
  ...
  methods: {
      ...
      axios.get(`${config.BASE_URL}/tmux/list`)
        .then(response => {
          if (response.data.status === 200) {
            ...
            this.$message.error('获取服务器列表失败: ' + response.data.msg);
          }
        })
        .catch(error => {
      ...
      axios.post(`${config.BASE_URL}/tmux/commands/detail`, { id: commandId })
        .then(response => {
          if (response.data.status === 200) {
            ...
            // 解析参数描述
            if (this.currentCommand.needs_params && this.currentCommand.param_desc) {
              try {
    ...
    localHandleCommandChange(commandId) {
      ...
      if (this.currentCommand && 
          (this.currentCommand.parameterized || this.currentCommand.needs_params) && 
    ...
    executeCommand() {
      ...
      // 如果命令有参数，添加参数
      if (this.currentCommand && this.currentCommand.parameterized && this.currentCommand.parameters) {
        // 将参数对象转换为数组
        this.currentCommand.parameters.forEach(param => {
          requestData.params.push(this.executeForm.params[param.name]);
        });
      }
