...
test('map presentation follows the 45-degree game camera and reduces icon clutter', () => {
...
test('formal map route and page use the authenticated v2 map contract', async () => {
  ...
  assert.match(page, /getScopedRuntimeOverview\(managementScopeTargetId\(managementScope\.value\)\)/)
  assert.match(page, /MANAGEMENT_SCOPE_CHANGED_EVENT/)
