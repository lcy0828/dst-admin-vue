...
test('system resources stay visible in the global header without duplicate dashboard requests', () => {
  ...
  assert.match(resourceStatus, /@update:open="handlePopoverOpenChange"/)
  assert.match(resourceStatus, /startSystemResourceDetailedPolling\(\)/)
  assert.match(resourceStatus, /stopSystemResourceDetailedPolling\(\)/)
  ...
  assert.match(resourceComposable, /SYSTEM_RESOURCE_DETAIL_REFRESH_INTERVAL_MS = 1_000/)
  assert.match(resourceComposable, /detailedPollingConsumers > 0/)
