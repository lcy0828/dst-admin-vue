...
const refreshInterval = fs.readFileSync(new URL('../src/components/layout/DashboardRefreshInterval.vue', import.meta.url), 'utf8')
const refreshIntervalComposable = fs.readFileSync(new URL('../src/composables/useDashboardRefreshInterval.js', import.meta.url), 'utf8')
...
test('global refresh interval controls system resources and room world data', () => {
  ...
  assert.match(refreshIntervalComposable, /DASHBOARD_REFRESH_INTERVAL_MS = 5_000/)
  assert.match(refreshIntervalComposable, /DASHBOARD_REFRESH_INTERVAL_OPTIONS_MS = Object\.freeze\(\[1_000, 5_000, 10_000, 30_000\]\)/)
  assert.match(refreshIntervalComposable, /refreshIntervalMs = ref\(readStoredRefreshInterval\(\)\)/)
  assert.match(resourceComposable, /useDashboardRefreshInterval\(\)/)
  assert.match(resourceComposable, /refreshSystemResourceStatus\(\{ silent: true \}\)/)
  assert.match(refreshInterval, /<Select v-model="selectedInterval">/)
  assert.match(refreshInterval, /<SelectGroup>/)
  assert.match(refreshInterval, /useDashboardRefreshInterval/)
  assert.match(refreshInterval, /dashboard\.resources\.refreshIntervalAria/)
  assert.match(layout, /<GlobalJobStatus\s*\/>[\s\S]*?<DashboardRefreshInterval\s*\/>[\s\S]*?<LanguageSwitch\s*\/>/)
  assert.match(serverWorkspace, /useDashboardRefreshInterval\(\)/)
