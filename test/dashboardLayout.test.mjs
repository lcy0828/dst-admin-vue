...
const resourceInterval = fs.readFileSync(new URL('../src/components/layout/SystemResourceRefreshInterval.vue', import.meta.url), 'utf8')
...
test('system resources stay visible in the global header without duplicate dashboard requests', () => {
  ...
  assert.match(resourceComposable, /SYSTEM_RESOURCE_REFRESH_INTERVAL_MS = 5_000/)
  assert.match(resourceComposable, /SYSTEM_RESOURCE_REFRESH_INTERVAL_OPTIONS_MS = Object\.freeze\(\[1_000, 5_000, 10_000, 30_000\]\)/)
  assert.match(resourceComposable, /refreshIntervalMs = ref\(readStoredRefreshInterval\(\)\)/)
  assert.match(resourceComposable, /refreshSystemResourceStatus\(\{ silent: true \}\)/)
  assert.match(resourceInterval, /<Select v-model="selectedInterval">/)
  assert.match(resourceInterval, /<SelectGroup>/)
  assert.match(resourceInterval, /dashboard\.resources\.refreshIntervalAria/)
