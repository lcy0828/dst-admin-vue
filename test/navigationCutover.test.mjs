...
test('runtime navigation keeps room control on the dashboard and redirects legacy server routes', async () => {
  const [router, navigation] = await Promise.all([
    source('src/router/index.js'),
    source('src/v2/navigation.js')
  ])

  assert.doesNotMatch(navigation, /to:\s*'\/servers\/workspace'/)
  assert.doesNotMatch(navigation, /to:\s*'\/servers\/list'/)
  assert.match(router, /path:\s*'workspace',[\s\S]*?redirect:\s*to\s*=>\s*\(\{\s*path:\s*'\/dashboard',\s*query:\s*to\.query,\s*hash:\s*to\.hash\s*\}\)/)
  assert.match(router, /path:\s*'list',[\s\S]*?redirect:\s*to\s*=>\s*\(\{\s*path:\s*'\/dashboard',\s*query:\s*to\.query,\s*hash:\s*to\.hash\s*\}\)/)
})
